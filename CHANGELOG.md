...
### Added
- `cstatic geo` audit subcommand (G17). A new CLI subcommand that scans the built `output/` directory and scores Generative Engine Optimization readiness 0–100, listing concrete remediation per check. It mirrors `cstatic check` (same exception-handling shape, same colored label style, same "exit non-zero on hard issues" semantics) and closes the loop on the 16 opt-in GEO features shipped in G1–G16 — authors no longer need to manually inspect output or hunt for warnings in build logs to verify their GEO configuration. The audit runs 9 checks against built files only (no behavior change to the build): (1) `llms.txt` present and non-trivial when `modules.llms_txt` is on; (2) AI crawler allowlist — verifies the expected AI-agent blocks exist in `robots.txt` for the configured `modules.robots_ai_crawlers_mode` (12 standard agents for `allow`/`disallow`, the custom list for `custom`); (3) JSON-LD validity — parses every `<script type="application/ld+json">` block across all `.html` files (tolerant of minifier-stripped attribute quotes) and runs each schema through `seo_schema::validate()` for required fields, with parse failures flagged as hard errors; (4) Organization consistency — re-runs `seo_schema::validate_organization()` and flags `name`/`logo`/`url` divergence across page-level Organization schemas; (5) Author profile pages — verifies each `<slug>.md` under `authors.dir` has a generated `/<authors_base>/<slug>/` page; (6) Citation tags — confirms article-typed pages carry `citation_author` + `citation_title` meta tags; (7) Passage index — confirms article pages have a non-empty `hasPart` array in their page schema (from G8); (8) `sitemap-ai.xml` present and well-formed when `modules.sitemap_ai` is on; (9) FAQ coverage — informational count of `FAQPage` schemas (0 weight). The score is `round(earned / max * 100)` over non-skipped checks; disabled features show as `[--]` and contribute to neither side. Hard issues (causing exit 1): missing `llms.txt`/`sitemap-ai.xml`/`robots.txt` when the corresponding feature is enabled, and unparseable JSON-LD. All other findings are warnings. No new config keys, no flags — the audit reads existing config and rescans every run (cheap, like `cstatic check`).
